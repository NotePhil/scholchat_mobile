...
interface RoleSettingsBodyProps {
  onLogout: () => void;
  roleLabel: string;
}

/** Shared "profile + logout" settings screen reused by Parent/Student/Establishment/Gestionnaire dashboards. */
const RoleSettingsBody = ({ onLogout, roleLabel }: RoleSettingsBodyProps) => {
  const { user } = useUser();
  const name = `${user?.prenom ?? ""} ${user?.nom ?? ""}`.trim() || user?.username || roleLabel;
...
export default RoleSettingsBody;
