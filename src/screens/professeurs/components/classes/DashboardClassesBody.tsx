...
const DashboardClassesBody = () => {
  ...
  const handleManageClass = async (classItem: UIClass) => {
  ...
  const [classesCache, setClassesCache] = useState<Map<string, CachedClassData>>(new Map());
  ...
  const loadClasses = async () => {
    ...
    try {
      ...
      if (classesData && Array.isArray(classesData)) {
        const formattedClasses: UIClass[] = await Promise.all(
          classesData.map(async (classItem): Promise<UIClass> => {
            ...
            try {
              ...
              const cachedData: CachedClassData = {
                classDetails,
                accessRequests: formattedAccessRequests,
                students: formattedStudents,
                parents: formattedParents,
  ...
  const handleCreateNewClass = async () => {
    ...
    try {
      ...
      const createdClass = await classService.createClass(classData as any);
      ...
      await classService.grantPublicationRights(user?.userId as string, createdClass.id, true, true);
      console.log('Publication rights granted successfully');

      Alert.alert('Succès', 'Classe créée avec succès!');

      // Add to local state for immediate UI update
      const newClass: UIClass = {
        id: createdClass.id,
        name: createdClass.nom as string,
        level: createdClass.niveau as string,
        state: createdClass.etat as string,
        studentsCount: 0,
        parentsCount: 0,
        creationDate: createdClass.dateCreation as string,
        description: (createdClass.description as string) || "",
