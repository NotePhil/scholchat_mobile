...
const AuthModal = ({ visible, onClose, onNavigateToDashboard, onNavigateToAdmin, onSignUp }: AuthModalProps) => {
  const { login } = useUser();
  ...
  const [uploadStatus, setUploadStatus] = useState<{
    cniRecto: UploadFieldStatus;
    cniVerso: UploadFieldStatus;
    profilePhoto: UploadFieldStatus;
  }>({
    cniRecto: 'idle',
    cniVerso: 'idle',
    profilePhoto: 'idle'
  ...
  const handleProfessorSignup = async () => {
    ...
    try {
      ...
      const updatedProfessor = await authService.updateProfessorUrls(professorId, {
  ...
  const uploadFileToServer = async (
    ...
    await authService.uploadFile(presignedResponse.url, {
      uri: file.uri,
